# Trying to make a linked list algorithm, 
# The linked list has O(1) complexity for insertion and deletion, but O(n) for searching.
# Meanwhile Arrays have O(n) for insertion and deletion, but O(1) for searching.

def create_node(value):
    """
    This function creates a node that will initially be the last one in the linked list.
    """
...
def append(head, value):
    """
    This function adds a new element that will be the last node in the list until a new one is appended.
    """
...
def remove(head, value):
    """
    This function removes the chosen element from the linked list.
    """
...
def update(head, value, new_value):
    """
    This function receives the list, the target element, and the new value.
    It searches linearly for the target in the list; if found, it replaces the value with the new one.
    If not, it prints a message saying the element isn't in the list.
    """
...
def find(head, target):
    """
    This function searches linearly to check if the target is in the list.
    If it is, it returns a message with its position.
    If not, it returns a message saying it isn't present.
    """
